...
void changeTools(int sign) {
    m_actual += sign;
    if (m_actual > 7) { m_actual = 7; }
    if (m_actual < 0) { m_actual = 0; }
    signpm=0;
  }

void changeCorect(int sign) {
    m_corectsens += sign;
    if (m_corectsens > 100) { m_corectsens = 100; }
    if (m_corectsens < -100) { m_corectsens = -100; }
    signpm=0;
  }
...
void setnamelogin(String actualname) {
  ...
  case 1:
    ++kodascii;
    if (kodascii > 126) { kodascii = 32; }
    break;
  case -1:
    --kodascii;
    if (kodascii < 32) { kodascii = 126; }
    break;
  }
...
void lcdMenu(){
  ...
  rotatename = m_actualname.substring(rotateword);
     lcd.print(rotatename);
        ++rotateword;
        if(rotateword > m_actualname.length()-11){rotateword = 0;}
  }
  else{
...
public:
  ...
  void changeTools(int sign) {
    m_actual += sign;
    if (m_actual > m_max) { m_actual = m_max; }
    if (m_actual < m_min) { m_actual = m_min; }
    signpm=0;
  }
...
if(menuactualy){
  if(menuactualy2){menuactualy2=false;}
  else{
    if(menuactualy1){menuactualy1=false;}
    else{
      if(menuactualy0){
        menuactualy0=false;
...
if(espactualy && !espactualy1 && !menuactualy0 && !setactualyTH){
  ...
  Blynk.virtualWrite(V3, 1); 
    ...
    EEPROM.write(279, 1);
    if(millis()-staeeprom > 3000){  //затримка для запису eeprom
              int delay_eeprom;
 ...
 switch(numset){
    ...
    case 13:  
       savesetting.lcdTempHour();
         if(changactualyTH1){
             if(setting1.notequallyTempHour()){boolsavesetting=true;}
             if(setting2.notequallyTempHour()){boolsavesetting=true;}
             if(setting3.notequallyTempHour()){boolsavesetting=true;}
             if(setting4.notequallyTempHour()){boolsavesetting=true;}
             if(setting5.notequallyTempHour()){boolsavesetting=true;}
             if(setting6.notequallyTempHour()){boolsavesetting=true;}

             if(setting11.notequallyTempHour()){boolsavesetting=true;}
             if(setting12.notequallyTempHour()){boolsavesetting=true;}
             if(setting13.notequallyTempHour()){boolsavesetting=true;}
             if(setting14.notequallyTempHour()){boolsavesetting=true;}
             if(setting15.notequallyTempHour()){boolsavesetting=true;}
             if(setting16.notequallyTempHour()){boolsavesetting=true;}
...
void pinchange(){
  ...
  lcd.print("Enter PIN change");
   ...
   Tyear.setsetting(now.year());  
        ...
        if(kmemory==21){pinsetting.setnamelogin("");}
      }
      else{
        menuactualy1=false;
        k=kmemory;
...
void buttonsens(){
  ...
  if(touchRead(T0)>threshold && touchRead(T3)>threshold && touchRead(T4)>threshold 
  && touchRead(T8)>threshold && touchRead(T9)>threshold){  
       releasebotton=true;
       counttouch=0;       
  }
...
//*********************************************************************************************************
void searh18b20(){   //no metod (function)
...
void apartsetting(const TempHour& d1, const TempHour& d2, const TempHour& d3, const TempHour& d4
  , const TempHour& d5, const TempHour& d6, const TempHour& d11, const TempHour& d12
  , const TempHour& d13, const TempHour& d14, const TempHour& d15, const TempHour& d16) {
...
void printTempHour(const TempHour& d1, const TempHour& d2, const TempHour& d3, const TempHour& d4
  , const TempHour& d5, const TempHour& d6, const TempHour& d11, const TempHour& d12
  , const TempHour& d13, const TempHour& d14, const TempHour& d15, const TempHour& d16) {
  d1.printA();
  d2.printA();
...
void resetTempHour(TempHour& d1, TempHour& d2, TempHour& d3, TempHour& d4
  , TempHour& d5, TempHour& d6, TempHour& d11, TempHour& d12
  , TempHour& d13, TempHour& d14, TempHour& d15, TempHour& d16) {
  d1.resetA();
  d2.resetA();
  d3.resetA();
  d4.resetA();
...
//*************************************************************************************************
void printMenuTools(const MenuTools& onpump1, const MenuTools& onpump2, const MenuTools& offpump1
                    ,const MenuTools& offpump2, const MenuTools& minA, const MenuTools& maxA
                    , const MenuTools& gistereza) {
  onpump1.printA();
  onpump2.printA();
  offpump1.printA();
  offpump2.printA();
...
void menuset (MenuTools& onpump1, MenuTools& onpump2, MenuTools& offpump1
                    ,MenuTools& offpump2, MenuTools& minA, MenuTools& maxA
                    , MenuTools& gistereza)  {
  if(k>36){k=1;}
  if(k<1){k=36;}
   switch(k){
    ...
    case 35:    
      ...
      Testablish.lcdMenu();
          if(Testablish.getactual()==1){
              rtc.adjust(DateTime(Tyear.getactual()
              , Tmount.getactual()
              , Tdate.getactual()
              , Thour.getactual()
              , Tminut.getactual()
...
void logicSensRele(){
  ...
  if(!saveyes){    //не можна преривати EEPROM.commit()
    timeNow();
...
if(tboilroom<-116){
  ...
  if(bowoodstart){
    ++intwoodstart;
    Serial.println();
    Serial.print("  bowasrart= ");
    Serial.print(intwoodstart);
    Serial.println();
    if(intwoodstart>600){
        intwoodstart=0;
        bowoodstart=false;
...
if(inici){
  if(Toffpump1.getactual() < twood1 || Toffpump2.getactual() < twood2){
    bowoodstart=true;
  }
}
...
if((!boreletermostar && troomin < (float)(tempsetting)/10) || tboilroom < Tmin.getactual()){ //on
...
if((boreletermostar && troomin > (float)(tempsetting+Tgistereza.getactual())/10) 
  || bowoodstart || !permissionGazBlynk){//off
...
//ON OFF pump1. refresh t -> TimeWood2 for pump2
  ...
  if (floatTwood2 > Tonpump2.getactual()) {
    ...
    if (twood2three - twood2one > 0 && twood2three - twood2one <= (float)TgisterezaP1.getactual() / 10 && twood2one > 10
      && twood2three - floatTwood2 >= 0 && twood2three - floatTwood2 < (float)TgisterezaP2.getactual() / 10) {
      
      attempt2 += 1;      
    }
    else if(twood2three - twood2one <= 0 && twood2one > 10
      && twood2three - floatTwood2 >= 0 && twood2three - floatTwood2 < (float)TgisterezaP2.getactual() / 10){
